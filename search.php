...
$sql = "SELECT p.id, p.name, p.description, p.price, p.image_url, p.stock
        FROM products p
        {$whereSql}
        ORDER BY {$orderBy}";
...
<section class="max-w-6xl mx-auto px-4 py-8 space-y-6">
  <div>
    ...
    <p class="text-gray-500 mt-2">Tìm nhanh theo từ khóa, giá, tồn kho và sắp xếp kết quả.</p>
  </div>

  <form method="get" class="bg-white rounded-lg border border-gray-200 p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
    ...
    <div>
      ...
      <input
        ...
        step="0.01"
        name="min_price"
        value="<?= htmlspecialchars((string) ($_GET['min_price'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>"
        class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2"
      />
    </div>
    ...
    <div>
      ...
      <input
        ...
        step="0.01"
        name="max_price"
        value="<?= htmlspecialchars((string) ($_GET['max_price'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>"
        class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2"
      />
    </div>
    ...
    <label class="lg:col-span-5 inline-flex items-center gap-2 text-sm">
  ...
  <div class="flex items-center justify-between">
    ...
    <a href="/index.php" class="text-sm hover:opacity-80 transition">← Quay lại trang chủ</a>
  </div>

  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
    <?php if (!$products): ?>
      <div class="sm:col-span-2 lg:col-span-3 bg-white rounded-lg border border-gray-200 p-6 text-gray-500">
        Không có kết quả phù hợp với điều kiện tìm kiếm.
      </div>
    <?php endif; ?>
    ...
    <?php foreach ($products as $product): ?>
      <article class="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
        ...
        <div class="p-4 space-y-3">
          ...
          <p class="text-sm text-gray-500 min-h-[44px]"><?= htmlspecialchars((string) mb_strimwidth($product['description'], 0, 95, '...'), ENT_QUOTES, 'UTF-8'); ?></p>
          <div class="flex items-center justify-between">
